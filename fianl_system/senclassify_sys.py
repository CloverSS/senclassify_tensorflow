from bp_classify import BpClassify
from bp_test import BpTest
	...
	if mode=="1":  #cnn算法
		...
		if train=="1": #自定义数据集
			...
			if num_classes=="2":  #二分类
				...
				senclass=CNNTest(file_aim=file_aim,file_res=file_res,
				  file_tensor_model="D:/python/model/tensorflow/model_data_cnn_cl3.ckpt-20",
				  num_classes=int(num_classes))
		...
		if train=="1": #自定义数据集
			...
			if num_classes=="2":  #二分类
				senclass=BpTest(file_aim=file_aim,file_res=file_res,
				  ...
				  file_tensor_model="D:/python/model/tensorflow/model_bp_class2.ckpt-20",
				  num_classes=int(num_classes))
			else :  #三分类
				senclass=BpTest(file_aim=file_aim,file_res=file_res,
				  file_dict="D:/python/data/dict_class_3.pkl",
				  file_chi2_model="D:/python/data/model_chi2_bp3.pkl",
				  file_tensor_model="D:/python/model/tensorflow/model_bp_class3.ckpt-20",
				  num_classes=int(num_classes))
	quit=input("退出请输入q,否则输入任意值:")
	if quit=='q':
		flag=0
