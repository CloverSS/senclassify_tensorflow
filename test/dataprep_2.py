...
def train_neural_network(X, Y):
    ...
    with tf.Session() as session:
        ...
        for epoch in range(epochs):
            shuffle_indices = np.random.permutation(np.arange(len(train_data_x)))
            data_x = np.array(train_data_x)[shuffle_indices]
            data_y = np.array(train_data_y)[shuffle_indices]
        ...
        print('one-hot 准确率: {}'.format(accurqcy.eval({X:list(test_data_x), Y:list(test_data_y)})))
